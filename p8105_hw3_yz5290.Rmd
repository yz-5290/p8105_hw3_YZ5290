...
```{r}
zipcode=read_csv("data/zipcode.csv",na=c("NA",".",""))|> 
  janitor::clean_names()|> 
  select(-file_date,-state_fips) 
zori=read_csv("data/zori.csv",na=c("NA",".",""))|> 
  janitor::clean_names()|> 
...
```{r}
zip_df=
  left_join(zipcode,zori,by=c("zip_code"))|>
  relocate(zip_code)
##zip codes>116 times
zip_df|>
  drop_na(x2015_01_31:x2024_08_31)|>
  distinct(zip_code)|>
  nrow()
## zip codes < 10 times
new_zip=select(zori,zip_code,x2015_01_31:x2024_08_31)

filter(new_zip, rowSums(is.na(across(x2015_01_31:x2024_08_31)))>106)|>
  distinct(zip_code)|>
  nrow()
...
There are 48 zipcodes are observed 116 month and there are 26 zip code observed fewer than 10 months. 
...
#comment:
The table shows that  for more than high school, male and female are about equivalent and they are the largest group. Moreover, less than high school is the smallest group. 
The boxplot shows that for less than high school level people, they are generally younger. For more than high school people, they are generally younger. Female are generagarlly older in less than high school and high school equivalent group. 
...
#comment:
THe total dily acitivity generally decerase by age. For high schoo equivalent and more than high school, females tend to shower a shower total daily activity than male. for less than high school education leve, females shows a higher total daily acitivity from age 20 to age 50, than shows a lower total daily activity from age 50 to age 80. For morethan high school education level, both male and female has smaller range of total daily activity compared to other educaation level groups. 
